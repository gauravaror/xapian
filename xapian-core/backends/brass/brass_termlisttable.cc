...
void
BrassTermListTable::set_bigramlist(Xapian::docid did,
				 const Xapian::Document & doc,
				 brass_doclen_t doclen)
{
    LOGCALL_VOID(DB, "BrassTermListTable::set_bigramlist", did | doc | doclen);

    string tag;
    pack_uint(tag, doclen);

    Xapian::doccount bigramlist_size = doc.bigramlist_count();
    if (bigramlist_size == 0) {
	// doclen is sum(wdf) so should be zero if there are no terms.
	Assert(doclen == 0);
	Assert(doc.bigramlist_begin() == doc.bigramlist_end());
	add(make_bigramkey(did), string());
	return;
    }

    Xapian::BigramIterator t = doc.bigramlist_begin();
    if (t != doc.bigramlist_end()) {
	pack_uint(tag, bigramlist_size);
	string prev_term = *t;
    ...
    Assert(bigramlist_size == 0);
    add(make_bigramkey(did), tag);
}
